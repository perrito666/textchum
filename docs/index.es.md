...
- Una vista previa de Markdown en vivo junto al código fuente — parcheada
  en su sitio mientras se escribe, con desplazamiento sincronizado en
  ambos sentidos — véase [Markdown](markdown.md).
...
- Un cajón de navegación en cada ventana: documentos abiertos agrupados
  ...
  proyecto actual debajo — véase [El navegador](navigator.md).
- Configuración respaldada en JSON con una ventana de ajustes gráfica que
  escribe directamente en el archivo — incluida la elección de apariencia
  (sistema/clara/oscura): las ediciones a mano sobreviven y los archivos
  rotos recurren a los valores por defecto y se respaldan en lugar de
  pisarse — véase [Configuración](configuration.md).
...
Lo próximo, en orden aproximado: autocompletado, búsqueda en todo el
proyecto y apertura difusa de archivos, y restauración de sesión.
