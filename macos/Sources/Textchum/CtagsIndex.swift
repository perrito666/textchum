...
final class CtagsIndex {
    ...
    /// `ctags` if it is Universal Ctags (the JSON output format needs
    /// it — the BSD ctags macOS ships is not enough), else `uctags`.
    private func resolveBinary() -> String? {
        if let binary { return binary }
        let found = ["ctags", "uctags"].first { candidate in
            let process = Process()
            process.executableURL = URL(fileURLWithPath: "/usr/bin/env")
